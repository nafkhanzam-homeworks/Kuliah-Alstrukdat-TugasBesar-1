...
boolean Act_levelUp(Game* p, Act* act, int buildingId) {
    ...
    /* ALGORITMA */
    b = Building_getBuilding(p, buildingId);
    Act_addOps(act, new_OpsDefault(&armyCount(b))); armyCount(b) -= Building_getMaxArmy(type(b), level(b))/2; // Pengurangan jumlah pasukan
    ...
    while (list != NULL) { // iterasi sampai list operasi habis(pemakaian skill atau endturn)
        ...
        switch (type(ops)) {
            case -2: { // menambahkan status operasi pada list
                List_addLast((List*)target(ops), value(ops));
                break;
            }
...
boolean Act_do(Game* p, char* pcmd) {
    ...
    toLowerCase(pcmd, cmd); // mengubah element string kapital menjadi lowercase
